...
const Process = (student_operation_log) => {
  fs.writeFileSync('./timebase_page_turning.txt', '');
  //期間の設定(手動)
  ...
  //生徒のページ遷移分ループ
  fs.appendFileSync('./timebase_page_turning.txt', `操作数\n`);
  ...
  for(let u of time_table){
    fs.appendFileSync('./timebase_page_turning.txt', `${u},`);
  }

  fs.appendFileSync('./timebase_page_turning.txt', `\n`);
  for(let i = 0; i < page_operation.length; i++){
    fs.appendFileSync('./timebase_page_turning.txt', `${student_list[i]},`);
    for(let p = 0; p < time_table.length;p++){
      ...
      if(target){
        fs.appendFileSync('./timebase_page_turning.txt', `${target.page_num},`);
      }else{
        fs.appendFileSync('./timebase_page_turning.txt', `欠,`);
      }
    }
    fs.appendFileSync('./timebase_page_turning.txt', '\n');
  }
