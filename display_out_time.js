...
const Process = (student_operation_log) => {
  fs.writeFileSync('./display_out_time.txt', '');
  //期間の設定(手動)
  ...
  //生徒のページ遷移分ループ
  fs.appendFileSync('./display_out_time.txt', `操作数\n`);
  for(let i of student_operation_log){
    ...
    page_operation[page_operation.length - 1][page_operation[page_operation.length - 1].length - 1].display_out = i.count_staytime > 60 ? 60 : i.count_staytime;
  ...
  for(let u of time_table){
    fs.appendFileSync('./display_out_time.txt', `${u},`);
  }

  fs.appendFileSync('./display_out_time.txt', `\n`);
  for(let i = 0; i < page_operation.length; i++){
    fs.appendFileSync('./display_out_time.txt', `${student_list[i]},`);
    for(let p = 0; p < time_table.length;p++){
      ...
      if(target){
        fs.appendFileSync('./display_out_time.txt', `${target.display_out},`);
      }else{
        fs.appendFileSync('./display_out_time.txt', `欠,`);
      }
    }
    fs.appendFileSync('./display_out_time.txt', '\n');
  }
