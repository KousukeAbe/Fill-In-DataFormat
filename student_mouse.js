...
const Process = (student_operation_log) => {
  ...
  //生徒のページ遷移分ループ
  fs.appendFileSync('./mouse.txt', `操作数\n`);
  for(let i of student_operation_log){
    ...
    page_operation[page_operation.length - 1][page_operation[page_operation.length - 1].length - 1].mouse = i.count_mouseup;
  ...
  for(let u of time_table){
    fs.appendFileSync('./mouse.txt', `${u},`);
  }

  fs.appendFileSync('./mouse.txt', `\n`);
  for(let i = 0; i < page_operation.length; i++){
    fs.appendFileSync('./mouse.txt', `${student_list[i]},`);
    for(let p = 0; p < time_table.length;p++){
      ...
      if(target){
        fs.appendFileSync('./mouse.txt', `${target.mouse},`);
      }else{
        fs.appendFileSync('./mouse.txt', `欠,`);
      }
    }
    fs.appendFileSync('./mouse.txt', '\n');
  }
