...
const Process = (student_operation_log) => {
  ...
  //生徒のページ遷移分ループ
  fs.appendFileSync('./keyup.txt', `操作数\n`);
  for(let i of student_operation_log){
    ...
    page_operation[page_operation.length - 1][page_operation[page_operation.length - 1].length - 1].keyup = i.count_key;
  ...
  for(let u of time_table){
    fs.appendFileSync('./keyup.txt', `${u},`);
  }

  fs.appendFileSync('./keyup.txt', `\n`);
  for(let i = 0; i < page_operation.length; i++){
    fs.appendFileSync('./keyup.txt', `${student_list[i]},`);
    for(let p = 0; p < time_table.length;p++){
      ...
      if(target){
        fs.appendFileSync('./keyup.txt', `${target.keyup},`);
      }else{
        fs.appendFileSync('./keyup.txt', `欠,`);
      }
    }
    fs.appendFileSync('./keyup.txt', '\n');
  }
