...
const Process = (blank, student_operation_log, teacher_operation_log) => {
  ...
  fs.appendFileSync('./student_exercise_answertime.txt', `時間\n`);
  ...
  for(let i of target_brank){
    fs.appendFileSync('./student_exercise_answertime.txt', `${i.id},`);
  ...
  for(let i of student_operation_log){
    ...
    if(i.student_number != current_student_number){
      ...
      for(let i of target_brank)fs.appendFileSync('./student_exercise_answertime.txt', `${answer_count[0][i.id] / 1000},`);
      fs.appendFileSync('./student_exercise_answertime.txt', `\n`);
      fs.appendFileSync('./student_exercise_answertime.txt', `${current_student_number},`);
...
const Boot = async () => {
  let blank_log = await Query('select * from blank where url = ?;', ['/documents/se3.pdf']);
