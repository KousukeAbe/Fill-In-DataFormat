...
const file_name = '../totals/page_turning_student.txt'
...
const Process = (student_operation_log, teacher_page_turning) => {
  ...
  //期間の設定(手動)
  let start_time = new Date('2019-11-27T15:00:00');
  ...
  for(let i = 0; i < page_operation.length; i++){
    ...
    for(let p = 0; p < time_table.length;p++){
      ...
      if(target){
        fs.appendFileSync(file_name, `${target.page_num},`);
        before_num = target.page_num;
      }else{
        // student用
        // fs.appendFileSync(file_name, `欠,`);
        // teacher用
        fs.appendFileSync(file_name, `${before_num},`);
      }
    }
    fs.appendFileSync(file_name, '\n');
  }
...
const Boot = async () => {
  ...
  let student_operation_log = await Query('select * from teacher_page_turning where url = "/documents/se9.pdf" and page_num > 0 order by id;', []);
